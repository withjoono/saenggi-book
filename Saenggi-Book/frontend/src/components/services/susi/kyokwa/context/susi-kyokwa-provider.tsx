...
const SusiKyokwaStepperContext = createContext<StepperContextProps | undefined>(
  undefined
);

const SusiKyokwaStepperProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
  ...
  return (
    <SusiKyokwaStepperContext.Provider value={value}>
      {children}
    </SusiKyokwaStepperContext.Provider>
  );
};

const useSusiKyokwaStepper = (): StepperContextProps => {
  const context = useContext(SusiKyokwaStepperContext);
  if (!context) {
    throw new Error(
      "useSusiKyokwaStepper must be used within a SusiKyokwaStepperProvider"
    );
  }
  return context;
};

export { SusiKyokwaStepperProvider, useSusiKyokwaStepper };
