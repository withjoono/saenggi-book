 ...
 * 2027학년도 수시 종합전형 API
 */
@ApiTags('susi-jonghap-2027')
@Controller('ms/jonghap/2027')
export class SusiJonghap2027Controller {
  constructor(private readonly service: SusiJonghap2027Service) {}

  // ========== 입시결과 (Ipkyul) ==========

  @Get('ipkyul')
  @Public()
  @ApiOperation({ summary: '종합전형 입시결과 목록' })
  ...
  async getIpkyulList(
    ...
    return this.service.getIpkyulList({
  ...
  async getIpkyulByIdaId(@Param('idaId') idaId: string) {
    return this.service.getIpkyulByIdaId(idaId);
  }
  ...
  @Get('recruitment')
  @Public()
  @ApiOperation({ summary: '종합전형 세부내역 목록' })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  @ApiQuery({ name: 'universityCode', required: false, type: String })
  @ApiQuery({ name: 'admissionType', required: false, type: String })
  ...
  @Get('special')
  @Public()
  @ApiOperation({ summary: '종합전형 특별전형 목록' })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  @ApiQuery({ name: 'admissionCategory', required: false, type: String, description: '일반/특별' })
  ...
  @Get('full/:idaId')
  @Public()
  @ApiOperation({ summary: 'ida_id로 종합전형 전체 정보 조회 (입시결과 + 세부내역 + 특별전형)' })
  @ApiParam({ name: 'idaId', description: 'IDA 고유 ID (예: 26-U001211)' })
